...
import com.divx.service.im.comm.HTTPMethod;
import com.divx.service.im.comm.Roles;
import com.divx.service.im.utils.HTTPClientUtils;
import com.divx.service.im.vo.ClientSecretCredential;
import com.divx.service.im.vo.Credential;
import com.divx.service.im.vo.EndPoints;
...
public class HXImUserHelper {
	private static final Logger LOGGER = LoggerFactory.getLogger(HXImUserHelper.class);
	...
	public static ObjectNode createNewIMUserSingle(ObjectNode dataNode) {
		...
		if (!HTTPClientUtils.match("^(?!-)[0-9a-zA-Z\\-]+#[0-9a-zA-Z]+", appKey)) {
			LOGGER.error("Bad format of Constants.APPKEY: " + appKey);

			objectNode.put("message", "Bad format of Constants.APPKEY");
			objectNode.put("code", ResponseCode.ERROR_INVALID_PARAMETER);
		...
		if (null != dataNode && !dataNode.has("username")) {
			...
			objectNode.put("code", ResponseCode.AUTH_ERROR_USERNAME_OR_PASSWORD_INVALID);
			return objectNode;
		}
	...
	public static ObjectNode updateIMUserNickname(String username, String nickname) {
		...
		try {
			...
			URL reqUrl = HTTPClientUtils.getURL(Constants.APPKEY().replace("#", "/") + "/users/"
					+ username);
			
		    objectNode = HTTPClientUtils.sendHTTPRequest(reqUrl, credential, dataNode,
	 ...
	 * @return
	 */
	private static ObjectNode createNewIMUserBatchGen(String usernamePrefix, Long perNumber, Long totalNumber) {
		ObjectNode objectNode = factory.objectNode();
	 ...
	 * @param 
	 * 	limit: page size
	 * 	cursor: empty for the start page
	 * 			nonempty for the cursor page.
	 * 
	 * @return
	 */
	public static ObjectNode getUsers(Integer limit, String cursor) {
		ObjectNode objectNode = factory.objectNode();
		...
		try {
			URL getUsersUrl = null;
			if (cursor != null && !cursor.isEmpty())
			{
				getUsersUrl = HTTPClientUtils.getURL(Constants.APPKEY().replace("#", "/") 
								+ "/users?limit=" + limit.toString() 
								+ "&cursor=" + cursor);
			}
			else
			{
			...
			ObjectNode body = factory.objectNode();
			objectNode = HTTPClientUtils.sendHTTPRequest(getUsersUrl, credential, body, HTTPMethod.METHOD_GET);
